...
function concatenateStrings(value1, value2) {
  return value1.concat('', value2).replaceAll('', '');
}
...
function isPalindrome(str) {
  const string = str.toLocaleLowerCase().replace(/[^a-z]/gi, '');
...
function encodeToRot13(str) {
  ...
  const resultArr = [];
  basicArr.forEach((el) => resultArr.push(arr2[arr1.indexOf(el)]));
